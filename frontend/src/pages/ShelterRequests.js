            ...
            auth.currentUser.getIdToken()
                .then(idToken => {
                    fetch('http://urandom.cloud:3000/apigetreqs', {
                        ...
                        .then(data => {
                            setItems(data.filter(item => item.shelter == auth.currentUser.email));
                            console.log(data);
                        })
                })
        })
            ...
            <main id="main">
                ...
                <section id="portfolio-details" class="portfolio-details">
                    <div class="container services">
                        <div class="row">
                            {items.map(item => <RequestCard title={item.name} volunteer={item.vname} />)}
                        </div>
                    </div>
                </section>
    ...
    return (
        <div class="col-md-6 col-lg-3 d-flex align-items-stretch mb-5 mb-lg-0">
            <div class="icon-box" data-aos="fade-up" data-aos-delay="100">
                ...
                <p>Taken by {props.volunteer == 'undefined' ? 'No one' : props.volunteer}</p>
                <button class="btn btn-success">Confirm reception</button>
            </div>
        </div>
    )
}
